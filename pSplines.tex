...
\title{Bayesian spectral density estimation using P-splines}
...
who used a smoothing prior on the log psd.  \cite{Rosen:2012} decompose the log psd into a linear and nonlinear component and  put a Gaussian process prior on the nonlinear part, \cite{Pensky:2007} propose Bayesian wavelet-smoothing of the log psd and recently, \cite{Cadonna2017} modelled the log psd  as a mixture of Gaussian distributions with frequency-dependent weights and mean functions.
%
...
$\bm{\xi}$ contains fixed equidistant internal knots on $[0,1]$ in our basic P-spline prior. A different  allotment of knots is described in  section ``Adaptive knots".
...
\begin{enumerate}
	...
	\item The adaptive knot vector is given by $\bm{\xi} = \left(F_{z}^{-1}(\xi^*_1),\dots, F_{z}^{-1}(\xi^*_{K^*}) \right)$.
...
This procedure was replicated for both equidistant and adaptive knots considering first and second order penalties.
...
A larger penalty order does not necessarily result in more accurate psd estimates. In this case of a relatively smooth spectral density, the adaptive knots do not yield an improvement over the equidistant knots and the BspDP prior.
...
For the AR(4) time series, the fit based on the P-spline prior with adaptive knots is better than the fit based on the B-spline Dirichlet process prior and the one based on the P-spline prior with equidistant knots.
% categorically outperforms the BspDP algorithm.   It also outperforms the equidistant knot scheme. 
 In general, the BspDP algorithm achieves higher accuracy than the P-spline method with equidistant knots.  As shown by \cite{Edwards2019}, the posterior distribution based on the BspDP prior yields better results than posterior based on the BDP in terms of median IAE in this example.  Consequently, our P-spline algorithm with adaptive knots outperforms the BDP method as well.
...
almost all the 300 independent analyses.  In general, the latter method shows diminishing  performance as the penalty degree increases.  For the AR(4) model, the P-spline method with equidistant knots performs poorly.  On the other hand, the adaptive knot scheme yields evidently much better results, which are comparable (in the case of the first order penalty) to those obtained via the BspDP algorithm.  %For $n=\{128,256\}$, the latter has higher coverage probabilities than our proposal, except for $n=512$.  
For the P-spline priors, the coverage probabilities decrease as the penalty degree order increases, which can be explained by the forced smoothness imposed by the higher penalty order.  As shown by \cite{Edwards2019}, the BspDP algorithm produces better results than the BDP prior in the AR(4) case, which coverage probability is zero across $n=\{128,256,512\}$. Consequently, our P-spline algorithm with adaptive knots outperforms the BDP method in this case.
...
Table~\ref{table:sim_covPbb_median},  only for a small fraction of the frequencies the psd is not covered by the uniform credible band.  In this context, BspDP and adaptive knot P-spline algorithms perform equally well.  
...
In general terms, the P-spline algorithm with our novel knot allocation scheme, the adaptive knots, offers the best trade-off between accuracy and computation cost.  The  rule-of-thumb criterion,  $K=\min\left\{n/4, 40\right\}$, for the number of B-spline densities in the P-spline prior algorithm worked perfectly well in this example. 
...
Again, the P-spline algorithm is run in two stages.  First, a Markov chain of 5,000 samples with a burn-in period of 1,000 and thinning factor of 10, resulting in 400 points, is used to calibrate the proposals of the final run.  Then, a Markov chain of 10,000 samples with a burn-in period of 2,000 and thinning factor of 10, that is 800 points, is used for the psd estimation.  We use 40 B-spline densities,  based on the rule-of-thumb criterion discussed above, and the first order penalty.  The whole process takes less than 1.5 minutes to run and is replicated for equidistant and adaptive knots. The results are displayed in Figure~\ref{fig:carinae}.

\begin{figure}[]
	\centering
	\includegraphics[scale=0.4,clip=true,angle=0]{carinae.pdf}
	\caption{Log-spectral density estimate for the variable star {\it S.\ Carinae} data based on the P-spline prior. The continuous grey line represents the periodogram whereas the continuous and dashed black lines stand for the posterior median log psd obtained using adaptive and equidistant knots, respectively.}
	\label{fig:carinae}
\end{figure}
...
in contrast to the adaptive knots.  This is because the periodogram-based knot allocation puts more knots in those regions which contain significant peaks as indicated by the periodogram.  The rest of peaks, which are smaller in comparison to the main one, are slightly better captured by the equidistant knots, since a large proportion of the adaptive knots have been allocated around the main peak. However, this can easily  be corrected by increasing the number of knots.  The psd estimate based on adaptive knots is comparable to the one obtained via the NPC approach \citep{Kirch:2018}.
...
Finally, we assess the fit of the posterior distribution based on the P-spline prior a under two different knot location schemes: equally spaced and adaptive knots.  For this, we estimated the psd for the {\it S.\ Carinae} time series which has a spectral density with several sharp peaks.  Equidistant knots failed to capture the peaks of this spectral density whereas adaptive knots significantly improved  the results without affecting the computational time.
...
In conclusion, the P-spline prior with the adaptive knot scheme offers a more computationally viable alternative to the BspDP prior for spectral density estimation.  As shown in this work, it can handle spectral densities with several peaks in a reasonable run-time.
