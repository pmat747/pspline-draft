...
\title{\textcolor{red}{Bayesian spectral density estimation using P-splines}}
...
using  a pilot run of \textcolor{red}{20,000} iterations with a burn-in period of 5,000 and thinning factor of 10 to calibrate the proposals.  The final sample consisted of \textcolor{red}{80,000} samples with a burn-in period of 5,000 and thinning factor of 10, resulting in \textcolor{red}{7,500} samples used for posterior inferences.  
%Therefore, the number of iterations equate the B-spline analysis.  
This procedure was replicated for both equidistant and \textcolor{red}{adaptive} knots \textcolor{red}{ considering first and second order penalties.}
...
In general terms, the P-spline algorithm with our novel knot allocation scheme, the \textcolor{red}{adaptive} knots, offers the best trade-off between accuracy and computation cost.  The  rule-of-thumb criterion,  $K=\min\left\{n/4, 40\right\}$, for the number of B-spline densities in the P-spline prior algorithm worked perfectly well in this example. 
...
The results for the BspDP and P-spline prior  with first order penalty and \textcolor{red}{adaptive}  knots are displayed in Figure~\ref{fig:sunspot} \textcolor{red}{(p-spline with equidistant knot approach results are not shown due to their similarity to the adaptive knot estimates)}.  Both methods reveal a large peak at about 0.09. In other words, there is a periodic solar cycle of length $1/ 0.09 \approx 11$ years.  These results are consistent with those obtained via the BDP  prior~\citep{Choudhuri:2004}.
However, the BspDP estimation takes approximately \textcolor{red}{22} minutes compared to only \textcolor{red}{4.5} minutes for the the P-spline estimation. Thus the P-spline estimation is approximately
...
Again, the P-spline algorithm is run in two stages.  First, a Markov chain of 5,000 samples with a burn-in period of 1,000 and thinning factor of 10, resulting in 400 points, is used to calibrate the proposals of the final run.  Then, a Markov chain of 10,000 samples with a burn-in period of 2,000 and thinning factor of 10, that is 800 points, is used for the psd estimation.  We use 40 B-spline densities,  based on the rule-of-thumb criterion discussed above, and the first order penalty.  The whole process takes less than \textcolor{red}{1.5} minutes to run and is replicated for equidistant and \textcolor{red}{adaptive} knots. The results are displayed in Figure~\ref{fig:carinae}.
...
We also assessed our proposal in the classic sunspot dataset, in which the \textcolor{red}{posterior distribution based on the BspDP prior }%B-spline prior algorithm
...
Finally, we assess the \textcolor{red}{ fit of the posterior distribution based on the} P-spline prior a under two different knot location schemes: equally spaced and \textcolor{red}{adaptive} knots.  For this, we estimated the psd for the {\it S.\ Carinae} time series which has a spectral density with several sharp peaks.  Equidistant knots failed to capture the peaks of this spectral density whereas  \textcolor{red}{adaptive} knots significantly improved  the results without affecting the computational time.
...
\textcolor{red}{The \textsf{R} package \texttt{psplinePsd} can be downloaded from \href{https://github.com/pmat747/psplinePsd}{GitHub}.}
