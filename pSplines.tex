...
\begin{enumerate}
	...
	\item Define $z_l = \frac{y_l}{\sum_{l=1}^{\nu} y_l}$, calculate its cumulative distribution $\widetilde{F}$, and interpolate it.
	\item Let $\bm{q} = \left(q_1,\dots,q_{K^*} \right)$ be a vector of $K^*$ quantiles, equally spaced in $[0,1]$.
	\item The Q-spaced internal knot vector is given by $\bm{\xi}^* = \left(\widetilde{F}^{-1}(q_1),\dots, \widetilde{F}^{-1}(q_{K^*}) \right)$. %where the inverse function is obtained by interpolation.	
\end{enumerate}
