...
This article proposes a Bayesian approach to estimating the spectral density of a stationary time series using a  P-spline prior.  Our proposal is motivated by the B-spline Dirichlet process prior of \cite{Edwards2018} combined with Whittle's likelihood with the aim of reducing the computational complexity of posterior computations. The strength of the B-spline Dirichlet process prior
over the Bernstein-Dirichlet process prior of \cite{Choudhuri:2004}  lies in its ability to estimate spectral densities with sharp peaks and abrupt changes due to the flexibility of B-splines with variable number and location of knots. Here, we fix the number and location of the knots and specify a P-spline prior.  
We demonstrate in a simulation study and two real  case studies that this approach retains the flexiblity of the B-splines, achieves the same ability
to accurately estimate peaks due to a new data-driven knot allocation scheme but significantly reduces the computational costs.
...
who used a smoothing prior on the log psd.  \cite{Rosen:2007} decompose the log psd into a linear and nonlinear component and  put a Gaussian process prior on the nonlinear part, \cite{Pensky:2007} propose Bayesian wavelet-smoothing of the log psd and recently, and \cite{Cadonna2017} modeled the log psd  as a mixture
of Gaussian distributions with frequency-dependent weights and mean functions.
%
\cite{Gangopadhyay:1999} modelled the log-spectral density function by  piecewise polynomials of low order between strategically placed knots on the support of the psd,  allowing the data to select the number and location of the knots.  A reversible jump Markov chain Monte Carlo (RJMCMC) \citep[RJMCMC;][]{Green:1995} algorithm was implemented to sample from the posterior distribution.
...
With the aim of reducing the computational cost of the BspDP prior approach,  we suggest to use the idea of P-splines \cite{Eilers:1996} that combine a large but fixed number of  B-splines with a simple difference penalty that controls the degree smoothness of the spectral density. P-splines, i.e.\  equally-spaced B-splines,  have been successfully implemented for frequentist nonparametric regression but to the best of our knowledge not yet for Bayesian spectral density estimation. We suggest 
...
\section{B-spline--Dirichlet process prior}
...
putting a penalty on the integrated squared second derivative  as a measure of roughness. P-splines avoid derivatives by expressing the penalty as the sum of squares of differences of the B-psline coefficients.
%The roughness penalty in a frequentist context is carried out by penalizing the likelihood function.  
In a Bayesian context, this penalty can be translated into a prior distribution for the $d^{\text{th}}$ order difference of successive coefficients, see e.g. \citep{Lang:2004}.
...
\end{align*}
where $\bm{v} = (v_1,\dots,v_{K-1})^\top$ is a $K-1$ dimensional parameter vector, $\phi$ is the smoothing or penalty parameter, $\textbf{P} = \textbf{D}^\top \textbf{D} +\epsilon \textbf{I}_{K-1}$ is the penalty matrix (which is a full matrix rank matrix for any small quantity $\epsilon$, for instance, $10^{-6}$) with $\textbf{D}$ the $d^{\text{th}}$ order difference matrix ($d=1,2,3,\ldots)$, $\alpha_{\phi}$ and $\alpha_{\delta}$ are shape parameters, $\delta \beta_{\phi}$ and $\beta_{\delta}$ are rate parameters, and $\text{Gamma}(a,b)$ denotes a gamma distribution with mean $a/b$ and variance $a/b^2$.
...
\begin{enumerate}
	...
	\item Define $z_i = \left|\dfrac{x_i - \overline{x}}{S_x} \right|$ and calculate its distribution function $F_z = P(z \leq z_i)$.
...
We generated  300 time series autoregressive time series of order 1 and 4 with unit variance Gaussian innovations of length $n = \{128, 256, 512\}$.
...
As shown by \cite{Edwards2018}, the BspDP prior produces better results than the BDP in terms of median IAE in this example.  Consequently, our P-spline algorithm outperforms the BDP method as well.
...
As in \cite{Edwards2018}, the MCMC algorithm based on the BspDP prior  is run for 100,000 iterations, with a burn-in period of 50,000 and thinning factor of 10.  Thus, 5,000 samples are used for the estimation.  This took around 32 minutes.  The other specifications are the same as those used in the simulation study.

The MCMC algorithm based on the P-spline prior is run in two stages.  In  the first stage, the proposal  is calibrated for the final run.  It consists of 25,000 iterations with a burn-in period of 10,000 and thinning factor of 10, resulting in 1,500 samples.  Then the algorithm is run for 75,000 iterations with a burn-in period of 25,000 and thinning factor of 10, resulting in 5,000 samples.  The analysis is replicated for the first, second and third order penalties and for equidistant and P-spaced knots.   All other specifications are the same as those used in the simulation study.  A single analysis takes around 6 minutes.
...
The P-spline prior can be regarded as a special case of the B-spline prior.  The latter works with a variable number and location of knots, which are driven by the data.  The P-spline prior assumes a fixed number of knots  with fixed locations.  These simplifications reduce the computational effort significantly, without sacrificing accuracy as shown in the application section.  To deal with abrupt peaks in the psd, we propose to locate the knots according to the peaks detected via the periodogram.  This approach does not affect the computational time and improve the estimates dramatically. This also indicates that the BspDP computation time could be reduced by the choice of P-spaced knots as initial values.

In our simulation study, we showed that, in general, the P-spline prior outperforms the BspDP prior in all our assessments, that is in terms of IAE, coverage probabilities, and run-times.  From the simulation study, it seems that a second order penalty P-spline works reasonably well.
